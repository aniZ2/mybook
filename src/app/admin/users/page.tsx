...
type UserRow = { uid:string; email?:string; displayName?:string; disabled:boolean; admin:boolean; stripeCustomerId?:string|null };
...
export default function AdminUsers(){
  ...
  const load = async (token?: string | null) => {
    setBusy(true);
    const u = auth.currentUser; if (!u) { alert('Sign in'); setBusy(false); return; }
    const idToken = await u.getIdToken(true);
    const qs = new URLSearchParams(); if (token) qs.set('pageToken', token);
    const r = await fetch('/api/admin/users/list?'+qs.toString(), { headers: { authorization: 'Bearer '+idToken } });
    const j = await r.json();
    if (!r.ok) { alert('Error: '+(j.error||'unknown')); setBusy(false); return; }
    setRows(j.users||[]); setPageToken(j.nextPageToken||null); setBusy(false);
  };
  ...
  const toggleAdmin = async (uid:string, newVal:boolean)=>{
    ...
    if (!r.ok){ const j = await r.json().catch(()=>({})); alert('Error: '+(j.error||r.statusText)); return; }
  ...
  return (
    <div className="panel">
          ...
          <tbody>
            {rows.map(u=>(
              <tr key={u.uid} style={{borderBottom:'1px solid #1e2230'}}>
                ...
                <td>
                  <button className="btn" onClick={()=>toggleAdmin(u.uid, !u.admin)}>{u.admin ? 'Remove admin' : 'Make admin'}</button>
                </td>
              </tr>
            ))}
          </tbody>
      ...
      <div style={{marginTop:'.6rem', display:'flex', gap:'.6rem'}}>
        <button className="btn" disabled={!pageToken || busy} onClick={()=>load(pageToken!)}>Next Page</button>
        <button className="btn" onClick={()=>load(null)} disabled={busy}>Refresh</button>
      </div>
    </div>
  )
}
