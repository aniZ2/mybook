...
export default function AdminExports(){
  ...
  const run = async ()=>{
    const u = auth.currentUser; if (!u) return alert('Sign in');
    const tok = await u.getIdToken(true);
    const r = await fetch('/api/admin/exports', {
      method:'POST', headers: { 'content-type':'application/json', authorization: 'Bearer '+tok },
      body: JSON.stringify({ collections: collections.split(',').map(s=>s.trim()).filter(Boolean), format })
    });
  ...
  const download = (col:string)=>{
    ...
    a.href = url; a.download = `${col}.${format==='csv'?'csv':'ndjson'}`; a.click();
  ...
  return (
    <div className="panel">
      ...
      <div style={{display:'flex', gap:'.6rem', flexWrap:'wrap'}}>
        <input className="input" value={collections} onChange={e=>setCollections(e.target.value)} style={{minWidth:360}} />
        ...
        <div style={{marginTop:'.6rem'}}>
          ...
          <ul>
            {Object.keys(result.results).map(col=>(
              <li key={col}><button className="btn" onClick={()=>download(col)}>{col}</button></li>
            ))}
          </ul>
        </div>
      )}
